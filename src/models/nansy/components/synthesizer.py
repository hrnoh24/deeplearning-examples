...
if __name__=="__main__":
  ...
  synthesizer = hydra.utils.instantiate(cfg.synthesizer)

  frame_lv_feats = torch.rand(4, 128, 50)
  pitch = torch.ones(4, 50) * 65
  p_amp = torch.ones(4, 50) * 0.5
  ap_amp = torch.ones(4, 50) * 0.5

  excitation, signal = synthesizer(pitch, p_amp, ap_amp, frame_lv_feats)
  print(excitation.size(), signal.size())
