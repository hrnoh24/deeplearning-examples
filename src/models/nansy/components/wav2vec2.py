...
class Wav2Vec2Wrapper(nn.Module):
    ...
    def forward(self,
        ...
        if audiolen is None:
            ...
            audiolen = torch.ceil(audiolen / self.sr *
                                  self.sr_w2v2).to(torch.long)
        # [B, T]
        mask = (
        ...
        # [B]
        var = ((audio - mean[:, None]) *
               mask).square().sum(dim=-1) / audiolen.to(torch.float32)
...
if __name__ == "__main__":
  ...
  print("Downsampling rate :", wav.size(1) / feats.size(1))
