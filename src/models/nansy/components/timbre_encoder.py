...
class MultiheadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # [B, H, hiddens // H, T]
        queries = self.proj_query(queries).view(
            bsize, self.heads, -1, querylen)
        # [B, H, S, T]
        score = torch.matmul(keys.transpose(
            2, 3), queries) * (self.channels ** -0.5)
        if mask is not None:
...
class TimbreEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.preblock = nn.Sequential(
            nn.Conv1d(in_channels, channels, prekernels,
                      padding=prekernels // 2),
            nn.ReLU(),
            nn.BatchNorm1d(channels))
        ...
        self.post_mha = MultiheadAttention(
            hiddens, hiddens, latent, timbre, latent, heads)
        ...
        self.sampler = MultiheadAttention(
            timbre, timbre, contents, timbre, latent, heads)
    ...
    def sample_timbre(self,
        ...
        # [B, 1, T]
        theta = torch.matmul(
            global_[:, None], sampled).clamp(-1 + eps, 1 - eps).acos()
        # [B, 1, T], slerp
...
if __name__ == "__main__":
  ...
  timbre_encoder = hydra.utils.instantiate(cfg.timbre_encoder)

  x = torch.rand(4, 80, 50)
  contents = torch.rand(4, 323, 50)
  gte, ttks = timbre_encoder(x)
  print("global timbre embedding size :", gte.size())
  print("timbre tokens size :", ttks.size())

  sampled = timbre_encoder.sample_timbre(contents, gte, ttks)
  print("sampled timbre :", sampled.size())
